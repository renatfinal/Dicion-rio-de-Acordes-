...
import { Search, Music, Piano, Guitar, BookOpen, Volume2, AudioLines, Info } from 'lucide-react';
import {
  obterNotaInterna,
  obterNomePorIndice,
  TIPOS_ACORDES,
    ...
    <>
      {ESCALAS.map((escala, i) => (
        <div key={i} className="bg-neutral-900 border border-neutral-800 border-l-4 border-l-cyan-500 rounded-2xl p-5 shadow-lg">
          <h4 className="text-lg font-bold text-amber-400 mb-4">{tom.nome} {escala.nome}</h4>
          <div className="flex flex-wrap gap-2.5">
            {escala.intervalos.map((intervalo, idx) => {
              const notaIndice = (tom.indice + intervalo) % 12;
              let label = obterNomePorIndice(notaIndice, accidental);
